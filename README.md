...
支援スタッフへのヒアリングを通じて業務上の要望を確認し、別のメンバーが作成した要件定義書をもとに、本ツールを実装しました。大まかな処理フローは定められていましたが、各処理の具体的な実現方法や実装方法については、自身で検討・設計しました。
...
処理内容ごとにUiPathのワークフローを分割して実装しています。

### Main1：公式URLの検索・Gemini APIによる判定

1. Excelから対象機関の情報を1件ずつ読み込む
2. Google検索を実行し、検索結果から候補URLを取得
3. 候補URLのページへアクセス
4. ページ本文を取得
5. 取得したページ情報と対象機関の情報をGemini APIへ送信
6. 対象機関の公式ページとして適切かを判定
7. 判定結果から公式URLを取得
8. URLをExcelへ書き込む
9. ハイパーリンクを設定し、列幅・行高などを整形
10. 更新したExcelファイルを保存

### Main2：東京都福祉局の公開資料取得・PDFからExcelへの変換

1. 東京都福祉局の公開ページへアクセス
2. 虐待防止・苦情相談関連の公開資料を取得
3. PDF資料をダウンロード
4. PDFの内容をExcelで処理できる形式へ変換
5. 後続処理で使用するExcelファイルとして保存

### Main3：Google Driveへのアップロード

1. 更新済みのExcelファイルを取得
2. Google Driveへ接続
3. 指定した保存先へ更新済みファイルをアップロード

### MainALL：一連の処理を実行

Main1・Main2・Main3の各ワークフローを組み合わせ、資料取得から情報更新、Google Driveへのアップロードまでの一連の処理を実行します。`
