...
`UiPath.GSuite.Activities.Drive.Triggers.FileUpdated`

Trigger activity that fires when a file in a Google Drive folder is updated.
...
|------|-------------|------|------|----------|---------|-------------|
| `BrowserLocation` | Folder | `Property` | `string` | No | | The folder ID to monitor, selected via browser. |
...
| `Filter` | Filter | `Property` | `TriggerFileFilterCollection` | No | | Conditions to filter which file update events to respond to. |
...
|------|-------------|------|------|-------------|
| `Result` | File | `OutArgument` | [`GDriveRemoteItem`](types/GDriveRemoteItem.md) | The updated file. |
