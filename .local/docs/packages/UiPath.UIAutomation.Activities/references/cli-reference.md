...
Replaces multiple Object Repository elements from definition files in a single batch.

```bash
uip rpa uia object-repository replace-elements [options]
```

| Option | Flags | Description | Required |
|--------|-------|-------------|----------|
| `elements` | `--elements <json>` | JSON array of replace entries. Each entry has the shape `{"referenceId":"REF","definitionFilePath":"path/to/element.xaml"}`. `referenceId` identifies the existing Object Repository element to replace; `definitionFilePath` points to the definition to replace it with. Both are required per entry. | **Yes** |

**Example:**

```bash
# Replace a single element from its definition file
uip rpa uia object-repository replace-elements \
  --elements '[{"referenceId":"elem-789","definitionFilePath":"path/to/username.xaml"}]'

# Replace multiple elements in a single batch
uip rpa uia object-repository replace-elements \
...
| Option | Flags | Description | Required |
|--------|-------|-------------|----------|
| `elements` | `--elements <json>` | JSON array of link entries. Each entry has the shape `{"workflowFilePath":"Main.xaml","activityId":"ACT","referenceId":"REF","targetProperty":"Target"}`. `workflowFilePath`, `activityId`, and `referenceId` are required. `targetProperty` is optional and defaults to `"Target"`; use dot-separated names for nested properties (e.g., `"SearchedElement.Target"`). | **Yes** |
...
| Option | Flags | Description | Required |
|--------|-------|-------------|----------|
| `workflow-file-path` | `--workflow-file-path <workflow-file-path>` | The path to the .xaml file. | **Yes** |
...
| `definition-file-path` | `--definition-file-path <definition-file-path>` | Path to the target definition XAML file. | **Yes** |
| `activity-id` | `--activity-id <activity-id>` | The activity reference ID available on the activity in XAML. | **Yes** |
| `workflow-file-path` | `--workflow-file-path <workflow-file-path>` | The path to the workflow file. | **Yes** |
...
Validates a target anchorable definition against the live application. Probes the live UI for the target and its anchors, then reports whether it matched. On a match, a screenshot with the matched element highlighted is saved and its path is printed. Works for selector-based and Computer Vision (CV) definitions alike.

```bash
uip rpa uia target-anchorable validate [options]
```

| Option | Flags | Description | Required |
|--------|-------|-------------|----------|
| `definition-file-path` | `--definition-file-path <definition-file-path>` | Path to the target anchorable definition file to validate. | **Yes** |
...
| `full-selector` | `--full-selector <full-selector>` | The full (strict) selector for the element. Also enables the strict selector search step and disables the fuzzy selector step. Cannot be combined with `--fuzzy-selector`. | No |
| `fuzzy-selector` | `--fuzzy-selector <fuzzy-selector>` | The fuzzy selector for the element. Also enables the fuzzy selector search step and disables the strict selector step. Cannot be combined with `--full-selector`. | No |
...
| `cv-text` | `--cv-text <cv-text>` | Override for the target's CV text (CV targets only). When the target has no CV text region, the element region is reused. | No |
| `cv-text-occurrence` | `--cv-text-occurrence <cv-text-occurrence>` | 1-based occurrence index used at runtime to disambiguate duplicate CV matches; `0` means "match any". The index counts **all** OCR text matches, including fuzzy ones — only use it when the ambiguous candidates carry the **same** text. | No |
| `cv-text-accuracy` | `--cv-text-accuracy <cv-text-accuracy>` | OCR text matching accuracy, greater than 0 and at most 1. The default allows fuzzy matches between similar texts — raise the value to require a near-exact `CvText` match when similar labels cause "N elements were found" ambiguity. | No |
| `validate` | `--validate` | Validate the target anchorable against the live application after the update completes. | No |
...
Resolve target anchorable definitions from snapshot refs (`w*`/`b*`/`e*`) or CV refs (`cve*` element / `cvw*` word), and write one definition file per entry. CV refs are resolved into CV-based definitions. The two ref kinds can be mixed in one call, and failures are reported per entry.

```bash
uip rpa uia target-anchorable resolve-defaults [options]
```

| Option | Flags | Description | Required |
|--------|-------|-------------|----------|
| `refs` | `--refs <json>` | JSON array of refs to resolve. Each entry has the shape `{"ref":"REF","definitionFilePath":"path/to/target.xaml","activityType":"Click"}`. `ref` is a default snapshot ref (e.g. `e28`) or a CV snapshot ref (e.g. `cve4`, `cvw3`). `activityType` is optional and may differ per entry. Available activity types: None, Click, TypeInto, GetText, Check, Hover, Highlight, SelectItem, GetAttribute, TakeScreenshot, DragAndDrop, KeyboardShortcut, MouseScroll, ExtractData, SetText, CheckState, FindElements, InjectJsScript, CheckElement. | **Yes** |
...
| `window-ref` | `--window-ref <window-ref>` | The reference id of the application window the element belongs to (for example `w1` or `b1`). Optional for selector-based refs — the window is located by its selector when omitted; required for CV refs (`cve*`/`cvw*`). | No |
| `source` | `--source <source>` | Which tree to use: `window` or `application`. | No |
| `from-snapshot` | `--from-snapshot` | Return the selector from the captured tree snapshot without probing the live element. Ignored for CV refs. | No |
...
# Resolve CV refs picked from cv-tree.md (requires a prior `snapshot capture --type cv`)
uip rpa uia target-anchorable resolve-defaults \
...
Resolve an anchor element from a default snapshot ref or CV snapshot ref and append it to the target anchorable definition. A target anchorable can hold up to four anchors (slots `0..3`); the new anchor is written to the next free slot and the command returns that slot index.
...
| Option | Flags | Description | Required |
|--------|-------|-------------|----------|
| `element-ref` | `--element-ref <element-ref>` | The default snapshot ref (e.g. `e28`) or CV snapshot ref (e.g. `cve4`, `cvw3`) of the anchor element to add. | **Yes** |
| `window-ref` | `--window-ref <window-ref>` | The reference id of the application window the anchor belongs to (for example `w1` or `b1`). Optional for selector-based refs — the window is located by its selector when omitted; required for CV refs (`cve*`/`cvw*`). | No |
...
| `description` | `--description <description>` | An optional description for the entry. | No |
| `validate` | `--validate` | Validate the target anchorable against the live application after the anchor is added. | No |
...
# Append a CV anchor resolved from a word ref (requires a prior `snapshot capture --type cv`)
uip rpa uia target-anchorable add-anchor \
  --element-ref "cvw3" \
  --window-ref "w1" \
  --definition-file-path "path/to/target.xaml" \
...
| `validate` | `--validate` | Validate the target anchorable against the live application after the anchor is removed. | No |
...
Update properties of the anchor at the specified index on the target anchorable definition. Only provided options are updated; omitted options are left unchanged.
...
| `name` | `--name <name>` | The display name for the anchor entry. | No |
| `description` | `--description <description>` | An optional description for the entry. | No |
| `full-selector` | `--full-selector <full-selector>` | The full (strict) selector for the anchor element. Also enables the strict selector search step and disables the fuzzy selector step. Cannot be combined with `--fuzzy-selector`. | No |
| `fuzzy-selector` | `--fuzzy-selector <fuzzy-selector>` | The fuzzy selector for the anchor element. Also enables the fuzzy selector search step and disables the strict selector step. Cannot be combined with `--full-selector`. | No |
| `cv-text` | `--cv-text <cv-text>` | Override for the anchor's CV text (CV anchors only). When the anchor has no CV text region, the element region is reused. | No |
| `cv-text-occurrence` | `--cv-text-occurrence <cv-text-occurrence>` | 1-based occurrence index used at runtime to disambiguate duplicate CV matches; `0` means "match any". The index counts **all** OCR text matches, including fuzzy ones — only use it when the ambiguous candidates carry the **same** text. | No |
| `cv-text-accuracy` | `--cv-text-accuracy <cv-text-accuracy>` | OCR text matching accuracy, greater than 0 and at most 1. Raise the value when fuzzy matching of similar labels causes ambiguity. | No |
| `validate` | `--validate` | Validate the target anchorable against the live application after the anchor is updated. | No |
...
# Update a CV anchor's text and pick the second on-screen occurrence
uip rpa uia target-anchorable update-anchor-definition \
  --definition-file-path "path/to/target.xaml" \
  --index 2 \
  --cv-text "Submit" \
...
| `validate` | `--validate` | Validate each fuzzified target anchorable against the live application after fuzzification completes. | No |
...
| `validate` | `--validate` | Validate each target anchorable against the live application after its anchor is fuzzified. | No |
...
```bash
uip rpa uia target-app resolve-defaults [options]
```
...
Validates a target application definition against the live desktop. Probes the live UI for the application window and reports whether it matched. On a match, a screenshot of the application is saved and its path is printed.

```bash
uip rpa uia target-app validate [options]
```

| Option | Flags | Description | Required |
|--------|-------|-------------|----------|
| `definition-file-path` | `--definition-file-path <definition-file-path>` | Path to the target application definition file to validate. | **Yes** |
...
|--------|-------|-------------|----------|
| `folder-path` | `--folder-path <folder-path>` | The folder path for input/output data. | **Yes** |
| `definition-file-path` | `--definition-file-path <definition-file-path>` | Path to the target definition XAML file. When omitted, a snapshot of the top-level windows is captured. | No |
| `type` | `--type <type>` | What to capture. `Default` captures the programmatic elements of the application. `CV` runs computer-vision analysis and captures only the elements visible in the view. Available values: Default, CV. Defaults to `Default`. | No |
| `framework` | `--framework <framework>` | The UI scanning framework to use for DOM snapshot creation. Available values: Default, UIA, AA, Java, AX. | No |
| `window-ref` | `--window-ref <window-ref>` | The reference id of the application window to capture (for example `w1` or `b1`). When omitted, the window is located using its selector. | No |
| `element-ref` | `--element-ref <element-ref>` | The reference id of the target element to highlight in the capture (for example `e1`). When omitted, the element is located using its partial selector. | No |

**Example:**

```bash
# Capture a snapshot scoped to a target definition (pin the exact window/element by ref)
...
# Capture a snapshot with the programmatic elements from the window's tree
uip rpa uia snapshot capture \
  --folder-path "path/to/output" \
  --definition-file-path "path/to/window.xaml" \
  --window-ref "w1"

# Capture the elements visible in the view (Computer Vision)
uip rpa uia snapshot capture \
  --folder-path "path/to/output" \
  --definition-file-path "path/to/window.xaml" \
  --type cv
...
```bash
uip rpa uia snapshot inspect [w/b-ref] [options]
```

| Option | Flags | Description | Required |
|--------|-------|-------------|----------|
| `w/b-ref` | `[w/b-ref]` (positional) | Window or browser tab target reference (e.g. `w1` or `b3`). Omit to list all top-level targets. | No |
| `framework` | `--framework <Default\|AA\|UIA\|Java\|AX>` | UI scanning framework. `Default` is recommended for most apps, especially browsers. [default: `Default`] | No |
...
Find elements in a window or browser tab by describing them in natural language. Recommended when the app is not supported by the tree-based snapshot flow. Returns up to 3 matches, each outlined on an annotated screenshot in a distinct colour and minted a semantic ref (`s1`, `s2`, …) for use with `interact click`/`type`/`hover`/`wheel`.
...
| Option | Flags | Description | Required |
|--------|-------|-------------|----------|
| `w/b-ref` | `<w/b-ref>` (positional) | Window or browser tab target reference (e.g. `w1` or `b3`). | **Yes** |
| `description` | `<description>` (positional) | Natural-language visual description of the element(s) to find (e.g. `"the red Submit button"`). | **Yes** |
| `exclude-color` | `--exclude-color <exclude-color>` | Highlight colour to avoid when an auto-picked one is hard to spot. Repeatable, at most 3 (e.g. `--exclude-color yellow --exclude-color green`). Values: `red`, `orange`, `yellow`, `green`, `blue`, `purple` (case-insensitive). | No |
| `visualize` | `--visualize` | Briefly highlight the matches on screen in their annotated colours. | No |

Matches are in no specific order — identify the right one visually by its outline colour (each match reports its colour name and hex). No matches and too-many-matches both exit non-zero; refine the description. s-refs are screen positions captured at inspect time: each run resets the previous ones, and they go stale if the layout changes.
...
# Several matches: tell them apart by outline colour, then act on the right one
uip rpa uia snapshot semantic-find w1 "row checkbox"
uip rpa uia interact click s2

# Re-run avoiding hard-to-spot highlight colours
...
Selector Intelligence commands for instructions and evaluation.
...
| `preserve-window-selector` | `--preserve-window-selector` | When set, the window selector is preserved and only the element selector is improved. | No |
...
Performs evaluation of a selector based on runtime data. The result reports `IsValid`, `MatchesOriginalTarget`, `MatchedWindowIds`, `FinalScore`, and `ToolingFeedback` per candidate.

- **With `--improve-selector-response-file-path`:** the candidate selectors in that response file are scored.
- **Without it:** the selector already stored in the definition file is evaluated. This is supported for **window selectors only** (a window-only definition such as `window.xaml`); for an element/full selector the response file is required.

```bash
uip rpa uia selector-intelligence evaluate [options]
```
...
| `folder-path` | `--folder-path <folder-path>` | The folder path for input/output data. | **Yes** |
| `definition-file-path` | `--definition-file-path <definition-file-path>` | Path to the target definition XAML file. | **Yes** |
| `improve-selector-response-file-path` | `--improve-selector-response-file-path <improve-selector-response-file-path>` | Improve selector response file to score. When omitted, the window selector stored in the definition file is evaluated instead. | No |
| `mode` | `--mode <mode>` | Evaluation mode: `recover` or `improve`. | No |

**Example:**

```bash
# Evaluate the window selector already stored in a window definition (no response file)
uip rpa uia selector-intelligence evaluate \
  --folder-path "path/to/data" \
  --definition-file-path "path/to/window.xaml" \
  --mode "improve"

# Evaluate candidate selectors from an improve-selector response file
uip rpa uia selector-intelligence evaluate \
  --folder-path "path/to/data" \
  --definition-file-path "path/to/target.xaml" \
  --mode "improve" \
...
Commands for driving live UI interactions against the current snapshot. Each verb operates on a reference (`wN`, `bN`, `eN`, or `sN`) produced by `uip rpa uia snapshot`.
...
Some verbs also accept semantic refs (`sN`). s-refs are screen positions from the last `semantic-find`: if an action changed the layout, rerun `uia snapshot semantic-find` before using them again.

**Input methods by platform.** `HardwareEvents`, `Simulate` and `WebBrowserDebugger` everywhere; `WindowMessages` Windows-only.
...
```bash
uip rpa uia interact click <e/s-ref> [options]
```

| Option | Flags | Description | Required |
|--------|-------|-------------|----------|
| `e/s-ref` | `<e/s-ref>` (positional) | Element or semantic reference from the current snapshot (e.g. `e42` or `s1`). | **Yes** |
...
```bash
uip rpa uia interact type <e/s-ref> <text> [options]
```

| Option | Flags | Description | Required |
|--------|-------|-------------|----------|
| `e/s-ref` | `<e/s-ref>` (positional) | Element or semantic reference (e.g. `e42` or `s1`). | **Yes** |
...
> **Note:** With an s-ref target, the element is not clicked/focused before typing unless `--click-before-mode` is set — keystrokes go to whichever element has keyboard focus. If the text landed in the wrong place, rerun with `--click-before-mode Single`.
...
```bash
uip rpa uia interact hover <e/s-ref> [options]
```

| Option | Flags | Description | Required |
|--------|-------|-------------|----------|
| `e/s-ref` | `<e/s-ref>` (positional) | Element or semantic reference. | **Yes** |
| `input-method` | `--input-method <HardwareEvents\|WindowMessages\|Simulate\|WebBrowserDebugger>` | Input method. [default: `HardwareEvents`] | No |
...
Works on option-list controls across UI stacks — native `<select>`, desktop combo boxes
(WinForms/WPF), Java and SAP lists, and web/ARIA comboboxes alike. **To tell whether a control is
`select`-drivable, don't inspect its `tag`/`role` — run `interact get <e-ref> items`:** if it returns
a non-empty option list, `select` will drive it (pass one of the returned values). That call returns
the full list even while the control is **closed**. `interact get <e-ref> selecteditem` /
`selecteditems` read the current value; confirm the result after selecting. (`items`, `selecteditem`,
and `selecteditems` are also included in `interact get-all <e-ref>`.)
...
```bash
uip rpa uia interact wheel <e/s-ref> [options]
```

| Option | Flags | Description | Required |
|--------|-------|-------------|----------|
| `e/s-ref` | `<e/s-ref>` (positional) | Element or semantic reference to scroll. | **Yes** |
...
> **Note:** Scrolling changes the layout, which invalidates all s-refs; rerun `uia snapshot semantic-find` afterwards before using them again.
...
Draw a colored border around the target for a fixed duration. Also writes a screenshot of the
target's window with the same box drawn on it, so you can confirm what was highlighted.
...
| `color` | `--color <Red\|Orange\|Yellow\|Green\|Blue\|Purple>` | Border color. [default: `Red`] | No |
| `visualize` | `--visualize` | Briefly highlight the target. | No |
...
# Cap the screenshot size to conserve tokens
```
...
```bash
uip rpa uia interact window close <w/b-ref> [options]
```

| Option | Flags | Description | Required |
|--------|-------|-------------|----------|
| `w/b-ref` | `<w/b-ref>` (positional) | Window or browser tab reference (e.g. `w1` or `b3`). | **Yes** |
| `visualize` | `--visualize` | Briefly highlight the target. | No |
...
```bash
uip rpa uia interact window foreground <w/b-ref> [options]
```

| Option | Flags | Description | Required |
|--------|-------|-------------|----------|
| `w/b-ref` | `<w/b-ref>` (positional) | Window or browser tab reference. | **Yes** |
| `visualize` | `--visualize` | Briefly highlight the target. | No |
...
```bash
uip rpa uia interact window maximize <w/b-ref> [options]
```
...
```bash
uip rpa uia interact window minimize <w/b-ref> [options]
```
...
```bash
uip rpa uia interact window restore <w/b-ref> [options]
```
...
```bash
uip rpa uia interact window hide <w/b-ref> [options]
```
...
```bash
uip rpa uia interact window show <w/b-ref> [options]
```
...
Launch the SAP Logon application and open a connection by name. On success it prints the opened session's `sapSysSessionId` (`sapSysSessionId=<id>`) — the server-side session identifier. To target the session, run `uip rpa uia snapshot inspect` and pick the window whose `sapSysSessionId` matches; that window's `wN` ref is what you pass to `sap login`.

```bash
uip rpa uia interact sap logon <connection> [options]
```

| Option | Flags | Description | Required |
|--------|-------|-------------|----------|
| `connection` | `<connection>` (positional) | Exact SAP connection name as shown in the SAP Logon / SAP Logon Pad window. | **Yes** |
| `sap-logon-path` | `--sap-logon-path <path>` | Path to the SAP Logon executable. [default: `saplogon.exe`, resolved via PATH] | No |
| `timeout` | `--timeout <ms>` | Timeout for opening the connection. [default: `60000`] | No |
| `retries` | `--retries <n>` | Retries to connect to the SAP Scripting interface. [default: `5`] | No |
| `delay-between-retries` | `--delay-between-retries <ms>` | Delay between retries. [default: `500`] | No |
...
Fill the SAP login screen (client / user / password / language) on an already-open SAP window — the `w-ref` found via `uip rpa uia snapshot inspect`, matching the window by the `sapSysSessionId` that `sap logon` printed. The password is **never** passed as a command-line value (which would leak into argv, shell history, and logs): supply it via stdin (piped from a file or secret store) **or** name an environment variable that holds it.
...
| Option | Flags | Description | Required |
|--------|-------|-------------|----------|
| `w-ref` | `<w-ref>` (positional) | SAP login window reference (a top-level `wN` ref). | **Yes** |
| `user` | `--user <name>` | SAP user name. | **Yes** |
| `password-stdin` | `--password-stdin` | Read the SAP password from standard input (pipe it from a file/secret store, not a literal). | One of these two¹ |
| `password-env` | `--password-env <VARNAME>` | Name of an environment variable holding the SAP password. The value is read from the environment, never the command line. | One of these two¹ |
| `client` | `--client <nnn>` | SAP client number (e.g. `100`). | **Yes** |
| `language` | `--language <xx>` | SAP logon language (e.g. `EN`). | **Yes** |
| `multi-logon-option` | `--multi-logon-option <Single\|Multiple\|Terminate>` | How to handle the "License Information for Multiple Logons" dialog. [default: `Single`] | No |
| `timeout` | `--timeout <ms>` | Timeout for the login. [default: `60000`] | No |
| `visualize` | `--visualize` | Briefly highlight the target. | No |
...
Run a transaction code in the open SAP session window (the `w-ref` from `snapshot inspect`). The okcd-aware equivalent of typing `/nXXXX` into the command field.
...
| Option | Flags | Description | Required |
|--------|-------|-------------|----------|
| `w-ref` | `<w-ref>` (positional) | SAP session window reference (a top-level `wN` ref). | **Yes** |
| `transaction` | `<transaction>` (positional) | Transaction code to run (e.g. `SE38`). | **Yes** |
| `new-session` | `--new-session` | Run the transaction in a **new** SAP session (the `/o` prefix). Omit to run it in the current session (the default, `/n`). | No |
| `timeout` | `--timeout <ms>` | Timeout. [default: `60000`] | No |
...
Read the active SAP session's status bar on the given window and print the message type / number / text.
