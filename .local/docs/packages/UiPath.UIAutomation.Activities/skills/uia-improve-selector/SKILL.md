...
- `--preserve-window-selector` -> `$PRESERVE_WINDOW_SELECTOR=true` (default: `false`). Keep the window selector unchanged and improve/recover only the element selector.
...
When `$PRESERVE_WINDOW_SELECTOR` is true, pass `--preserve-window-selector` so the window selector is left unchanged and only the element selector is improved/recovered:
...
This step runs an evaluate->fix loop. **Max 3 iterations total.**
...
If `$ATTEMPT >= 3`: save the last evaluation result and jump to **Output** with the errors.

Otherwise:

1. Read `$WORK_FOLDER/evaluation-result.txt` carefully -- focus on the `ToolingFeedback` for each candidate to understand what went wrong (schema violations, invalid attributes, missing required fields, etc.).
...
**If no valid selectors after 3 attempts:** present the last evaluation errors and stop improving attempts.
