...
**Commands that produce output print the output-file path to stdout — read the printed path.**

**If a prior step already captured a tree (e.g., `uia-configure-target`), grep it for the ref and call `uia interact` directly -- do NOT re-run `snapshot inspect`.** Only re-snapshot when the UI has actually changed since capture.

## Quick Start

```bash
# If a prior step already captured a tree, reuse those refs:
grep -n "Submit" tree.md                 # Find [ref=eN] for the target
"$CLI" interact click e42                # Use it directly

# Otherwise, start from a fresh inspection:
"$CLI" snapshot inspect                  # List top-level windows and browser tabs
"$CLI" snapshot inspect w1               # Inspect a window to get its UI tree
...
"$CLI" snapshot inspect w1               # Re-inspect after UI changes -- refs may be stale
...
- `[items deferred]` on a DropDown/List means the items aren't in the snapshot — they may still be retrievable via `"$CLI" interact get-all <ref>`. Relevant attributes: `items`, `selectedItem`, `selecteditems`.
...
Any w/b/e-ref from the most recently loaded snapshot is valid for `uia interact`, no matter which `uia snapshot` subcommand loaded it. Each new snapshot replaces all prior refs.

**Window refs (w1, w2) and browser refs (b1, b2)** are assigned by top-level `snapshot inspect`. Each call invalidates the prior w/b-refs.

**Element refs (e1, e2, e3...)** are assigned by `snapshot inspect <w/b-ref>`. Each call invalidates the prior e-refs.

**Semantic refs (s1, s2, s3...)** are assigned by `snapshot semantic-find <w/b-ref>`. Each call invalidates the prior s-refs. They are screen positions, not live elements: after a layout change they may silently hit the wrong place.
...
Use `--framework` with `snapshot inspect` to control how the UI tree is scanned. `Default` and `Java` work everywhere; `UIA` and `AA` are Windows-only, `AX` is macOS-only.

Default framework works for most apps. On Windows, use `--framework UIA` for:
...
DropDown and List elements might show options as children. Use the **parent's ref** and the option's text:
...
If the parent element is tagged `[items deferred]`, the items aren't in the snapshot but may still be retrievable:
...
If items are missing, click the element to expand it and retry -- some controls load children only when opened.
...
When no usable tree exists or the target is easiest to describe visually. Up to 3 matches in no specific order:
...
Rerun `semantic-find` after any action that changes the layout.
...
If the tree stays empty or unusable after framework changes, find the element by describing it:
...
No matches and too many matches both exit non-zero with a hint: refine the description (relative position, label text, surrounding elements).

**Dropdown/list options not visible**

If the control is tagged `[items deferred]`, try reading the items directly:
...
# Or, if still no items, try to snapshot again after the click:
...
**Logging in:** use the dedicated SAP verbs instead of hand-driving the SAP Logon pad. `sap logon` opens a connection by name and prints the opened session's `sapSysSessionId`. Then `snapshot inspect` and pick the window whose `sapSysSessionId` matches — that `wN` goes to `sap login`.

```bash
"$CLI" interact sap logon "MyConnection"     # opens the connection; prints "sapSysSessionId=<id>"
"$CLI" snapshot inspect                       # find the window whose sapSysSessionId matches → e.g. w1

# password via a named env var, set out-of-band in a separate step:
export SAP_PASSWORD="$(cat secret.txt)"
"$CLI" interact sap login w1 --user <USER> --client 100 --language EN --password-env SAP_PASSWORD
# or pipe the secret from a file via stdin:
...
```bash
"$CLI" interact click e7 --input-method Simulate                    # Click a SAP control (e.g. Execute / toolbar button) via the scripting API
```
...
**Status bar:** SAP confirms operations via the status bar (see the guide for message-type meanings). After an action:
