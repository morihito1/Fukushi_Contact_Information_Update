...
On macOS the primary shortcut modifier is **Command (⌘)**, not Control -- the macOS equivalents of `Ctrl+C` / `Ctrl+V` / `Ctrl+A` / `Ctrl+S` are `⌘C` / `⌘V` / `⌘A` / `⌘S`. `interact type` does not auto-convert Windows modifiers, so write the macOS token explicitly: `command` for ⌘, `option` for ⌥. `ctrl` (⌃ Control), `shift`, and all other keys use the same tokens as Windows.

| Action | Argument |
|--------|----------|
| ⌘A (select all) | `[d(command)]a[u(command)]` |
| ⌘C (copy) | `[d(command)]c[u(command)]` |
| ⌘V (paste) | `[d(command)]v[u(command)]` |
| ⌘S (save) | `[d(command)]s[u(command)]` |
| Clear field (⌘A then Delete) | `[d(command)]a[u(command)][k(del)]` |
| ⌘⇧T (reopen tab) | `[d(command)d(shift)]t[u(shift)u(command)]` |
| Move one word right (⌥→) | `[d(option)k(right)u(option)]` |
